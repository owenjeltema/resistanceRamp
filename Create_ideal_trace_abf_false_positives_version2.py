...
from Create_ideal_trace_abf_set_levels_version2 import voltage, duration, abf, window_width, yes, no, level, bin_mean, bin_index, bin_length, filtered_sweepY, net_mean, bins_list, time_step
...
if find_false_values == 1: #yes
    ...
    if change_scale== 'yes':
        ...
        abf_make_a_graph(new_window_width, ideal_Y, abf, filtered_sweepY, duration, bins_list)
...
if find_false_values== 0: #no
    ...
    while e < len(neg_spike_start):
        ...
        while c < len(bin_index)-1:
            ...
            if v > bin_index[c] and v < bin_index[c+1]:
                ...
                new_len_b= (round(neg_spike_stop[e]/  time_step)- round((neg_spike_start[e]/  time_step)))
    ...
    #starting at the index of the start time
        while v < (round((false_pos_start[e])/ time_step) + 5/time_step) and v < len(abf.sweepY):
            c=0
            while c < len(bin_index):
    ...
    while h < len(levels_from_list):
        ...
        while g < len(levels_from_list[h]):
            if levels_from_list[h][g] == None:
                amp= None
            if levels_from_list[h][g] != None:
                amp= abs(round((level_amplitude[levels_from_list[h][g]] - level_amplitude[h]),3))
    ...
    while h < len(levels_to_list):
        ...
        while g < len(levels_to_list[h]):
            if levels_to_list[h][g] == None:
                amp= None
            if levels_to_list[h][g] != None:
                amp= abs(round((level_amplitude[levels_to_list[h][g]] - level_amplitude[h]),3))
...
#and some of the same formatting to display them    
    ...
    while l < len(list_of_levels_zero):
        ...
        if l == len(list_of_levels_zero) or l > len(list_of_levels_zero) or list_of_levels_zero[l] != list_of_levels_zero[l-1]:
            l=l+1
...
#all the variables that will be displayed in the excel files
    abf_data= {'level': level, 'index': bin_index, 'start time (s)': start_time, 'length (s)':bin_length_sec, 'end time (s)': end_time, 'mean (pA)': bin_mean, 'transition from': transition_from, 'transition to': transition_to, 'levels': list_of_levels, 'level means (pA)': net_mean, 'average length (s)': avg_length_sec, 'level amplitude': level_amplitude, 'conductance': conductance, 'bins_list': bins_list, 'occurrences': occurrences, 'levels transition from' : levels_from_list, 'levels transitioned to': levels_to_list, 'amplitude from transition': trans_amp_from_list, 'amplitude to transition': trans_amp_to_list, 'total dwell time (s)': total_dwell_time, 'uncertainty total dwell time (s)': uncert_total_dwell_time, 'relative probability to level 1': relative_prob_to_level_1, 'uncertainty relative probability to level 1':uncert_relative_prob_to_level_1, 'mean dwell time (s)': mean_length_sec, 'stdev dwell time (s)': stdev_length_sec, 'S.E.M. dwell time (s)': sterrmean_length_sec, 'Level_1 dwell times (s)': opendwelltimes1_sec, 'Level_2 dwell times (s)': opendwelltimes2_sec, 'Level_3 dwell times (s)': opendwelltimes3_sec, 'Level_4 dwell times (s)': opendwelltimes4_sec, 'Level_5 dwell times (s)': opendwelltimes5_sec, 'Level_6 dwell times (s)': opendwelltimes6_sec, 'Level_7 dwell times (s)': opendwelltimes7_sec}
#    abf_data_zero = {'level': level_zero, 'index':index_zero, 'length (s)': length_zero, 'mean (pA)': mean_zero, 'levels': list_of_levels_zero, 'average length (s)': avg_length_zero, 'level means (pA)': net_mean_zero, 'level amplitude': level_amplitude_zero, 'occurrences': occurrences_zero}
    df1 = pd.DataFrame(abf_data, columns = ['level','index', 'start time (s)', 'length (s)', 'end time (s)', 'mean (pA)' , 'transition from', 'transition to'])
    df2 = pd.DataFrame(abf_data, columns = ['levels', 'average length (s)', 'level means (pA)', 'level amplitude', 'conductance', 'occurrences', 'levels transition from', 'levels transitioned to', 'amplitude from transition', 'amplitude to transition', 'total dwell time (s)', 'uncertainty total dwell time (s)', 'relative probability to level 1', 'uncertainty relative probability to level 1', 'mean dwell time (s)', 'stdev dwell time (s)', 'S.E.M. dwell time (s)'])
